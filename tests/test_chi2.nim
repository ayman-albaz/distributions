...
suite "Chi2Distribution":
  ...
  test "Chi2Distribution(3).mean()":
    let chi2_dist = initChi2Distribution[float64](3)
    check chi2_dist.mean().round(r1) == 3.0.round(r1)

  test "Chi2Distribution(1).median()":
    let chi2_dist = initChi2Distribution[float64](1)
    check chi2_dist.median().round(r1) == 0.45493642311957283.round(r1)

  test "Chi2Distribution(2).median()":
    let chi2_dist = initChi2Distribution[float64](2)
    check chi2_dist.median().round(r1) == 1.3862943611198906.round(r1)

  test "Chi2Distribution(3).median()":
    let chi2_dist = initChi2Distribution[float64](3)
    check chi2_dist.median().round(r1) == 2.3659738843753386.round(r1)
